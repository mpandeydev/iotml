...
z_conv1d_1_kernel = sess.run(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'foo/conv1d_1/kernel'))[0]
...
for i in range(0,1):
    rand_index = 0 #np.random.choice(9000, size = batch_size)
    print(x_vals[0,:10])
    rand_x = [x_vals[rand_index]]
    rand_y_s = [types[rand_index]]
                
    temp_loss,pred_training_s,c1_out = sess.run([loss,fprob,conv1d_1], feed_dict={x_data : np.array([rand_x]).reshape((batch_size,sample_length,input_channels)), y_target_s:rand_y_s} )
