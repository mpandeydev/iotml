...
def get_variable(layer,parameter):
    varpath = str('pruned/pruned/'+layer+'/'+parameter)
    variable = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,varpath)[0]
...
# Initialize all variables
    ...
    # Assign pruned weights to network
    
    if(assign):
        sess.run(tf.assign(tf.trainable_variables("pruned/conv1d_1/kernel")[0],
                           pruned_filters['conv1d_1_k']))
        sess.run(tf.assign(tf.trainable_variables("pruned/conv1d_2/kernel")[0],
                           pruned_filters['conv1d_2_k']))
        sess.run(tf.assign(tf.trainable_variables("pruned/conv1d_f/kernel")[0],
                           pruned_filters['conv1d_f_k']))
        sess.run(tf.assign(tf.trainable_variables("pruned/conv1d_1/bias")[0],
                           pruned_filters['conv1d_1_b']))
        sess.run(tf.assign(tf.trainable_variables("pruned/conv1d_2/bias")[0],
                           pruned_filters['conv1d_2_b']))
        sess.run(tf.assign(tf.trainable_variables("pruned/conv1d_f/bias")[0],
                           pruned_filters['conv1d_f_b']))
